...
export function renderButtonSmartMenu({
  containerUID,
  clientID,
  Menu,
}: SmartMenuProps): MenuComponentInstance {
  if (menu) {
    return menu;
  }
...
export function prerenderButtonSmartMenu({
  props,
  components,
}: {|
  props: ButtonProps,
...
export function showButtonSmartMenu({
  props,
  payment,
  components,
  choices,
}: ButtonMenuProps): ZalgoPromise<void> {
  const { clientID, uid: containerUID } = props;
  ...
  const smartMenu =
    menu || renderButtonSmartMenu({ containerUID, clientID, Menu });
  menu = smartMenu;
