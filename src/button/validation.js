...
function isValidMerchantIDs(
  merchantIDs: $ReadOnlyArray<string>,
  payees: $ReadOnlyArray<Payee>,
): boolean {
  ...
  const foundEmail = merchantEmails.every((email) => {
    return payees.some((payee) => {
      return (
        email ===
        (payee.email &&
          payee.email.stringValue &&
          payee.email.stringValue.toLowerCase())
      );
    });
  });
  ...
  const foundPayee = payees.every((payee) => {
    return (
      merchantIds.indexOf(payee.merchantId) > -1 ||
      merchantEmails.indexOf(
        payee.email &&
          payee.email.stringValue &&
          payee.email.stringValue.toLowerCase(),
      ) > -1
...
function triggerIntegrationError({
  error,
  message = error,
  orderID,
  loggerPayload = {},
  throwError = true,
  featureFlags,
}: TriggerIntegrationErrorOptions) {
  const shouldThrowError =
    throwError && featureFlags.shouldThrowIntegrationError;
  ...
  getLogger()
    ...
    .track({
      ...
      [FPTI_CUSTOM_KEY.INTEGRATION_WHITELIST]: shouldThrowError
        ? "false"
        : "true",
...
const VALIDATE_INTENTS = [INTENT.CAPTURE, INTENT.AUTHORIZE, INTENT.ORDER];
...
export function validateOrder(
  ...
  return getSupplementalOrderInfo(orderID).then((order) => {
    ...
    const cartIntent =
      cart.intent.toLowerCase() === "sale"
        ? INTENT.CAPTURE
        : cart.intent.toLowerCase();
    const initiationIntent =
      cart.supplementary?.initiationIntent?.toLowerCase() === "authorization"
        ? INTENT.AUTHORIZE
    ...
    if (!intentMatch && VALIDATE_INTENTS.indexOf(intent) !== -1) {
      triggerIntegrationError({
        ...
        message: `Expected intent from order api call to be ${intent}, got ${cartIntent}. Please ensure you are passing ${
          SDK_QUERY_KEYS.INTENT
        }=${
          initiationIntent || cartIntent
        } to the sdk url. https://developer.paypal.com/docs/checkout/reference/customize-sdk/`,
        featureFlags,
    ...
    if (
      !window.xprops.createBillingAgreement &&
      buttonLabel === BUTTON_LABEL.DONATE
    ) {
    ...
    if (cartBillingType && !vault && !window.xprops.createVaultSetupToken) {
      triggerIntegrationError({
        error: `smart_button_validation_error_billing_${
          cartAmount ? "with" : "without"
        }_purchase_no_vault`,
        message: `Expected ${
          SDK_QUERY_KEYS.VAULT
        }=${VAULT.TRUE.toString()} for a billing transaction`,
        featureFlags,
        orderID,
    ...
    if (vault && window.xprops.createVaultSetupToken) {
      ...
      !cartBillingType &&
      !window.xprops.createBillingAgreement &&
      !window.xprops.createSubscription &&
      !window.xprops.clientAccessToken &&
      !window.xprops.userIDToken
    ) {
      triggerIntegrationError({
        error: `smart_button_validation_error_vault_passed_not_needed`,
        message: `Expected ${
          SDK_QUERY_KEYS.VAULT
        }=${VAULT.FALSE.toString()} for a non-billing, non-subscription transaction`,
        featureFlags,
        orderID,
        loggerPayload: { vault, cartBillingType },
        throwError: false,
    ...
    if (
      ...
      !window.xprops.createVaultSetupToken &&
      !window.xprops.createSubscription
    ) {
      triggerIntegrationError({
    ...
    if (!payees) {
      return triggerIntegrationError({
        error:
          "smart_button_validation_error_supplemental_order_missing_payees",
        featureFlags,
        orderID,
        throwError: false,
      });
    ...
    for (const payee of payees) {
      if (!payee.merchantId && (!payee.email || !payee.email.stringValue)) {
        return triggerIntegrationError({
          error:
            "smart_button_validation_error_supplemental_order_missing_values",
          featureFlags,
          orderID,
        ...
        if (!isValidMerchantIDs(xpropMerchantID, uniquePayees)) {
          if (uniquePayees.length === 1) {
            triggerIntegrationError({
              ...
              message: `Payee(s) passed in transaction does not match expected merchant id. Please ensure you are passing ${
                SDK_QUERY_KEYS.MERCHANT_ID
              }=${payeesStr} or ${SDK_QUERY_KEYS.MERCHANT_ID}=${
                uniquePayees[0] &&
                uniquePayees[0].email &&
                uniquePayees[0].email.stringValue
                  ? uniquePayees[0].email.stringValue
                  : "payee@merchant.com"
              } to the sdk url. https://developer.paypal.com/docs/checkout/reference/customize-sdk/`,
              featureFlags,
