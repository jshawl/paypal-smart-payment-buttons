...
try {
  if (!window.paypal) {
    const script = querySelectorAll("script").find((el) =>
...
export function setupButton({
  ...
  getButtons().forEach((button) => {
    ...
    onElementClick(button, (event) => {
      ...
      if (eligibility.isServiceWorkerEligible) {
        ...
        getLogger()
          .info("SERVICE_WORKER_ELMO_CONTROL")
          .track({
      ...
      const paymentProps = getButtonProps({
        facilitatorAccessToken,
        brandedDefault,
  ...
  const setupPrerenderTask = initPromise.then(() => {
    return ZalgoPromise.hash({
      ...
      const button = document.querySelector(
        `[${DATA_ATTRIBUTES.FUNDING_SOURCE}=${paymentFundingSource}]`,
      );
      ...
      payPromise.catch((err) => {
        ...
        getLogger()
          .error("prerender_initiate_payment_reject", {
            err: stringifyError(err),
          })
          .track({
            [FPTI_KEY.STATE]: FPTI_STATE.BUTTON,
  ...
  const setupRememberTask = setupRemember({
    rememberFunding,
    fundingEligibility,
  });
  ...
  const setupExportsTask = setupExports({
    props,
    isEnabled,
    facilitatorAccessToken,
    fundingEligibility,
  ...
  const validatePropsTask = setupButtonLogsTask.then(() =>
    validateProps({
      intent,
      createBillingAgreement,
      createSubscription,
