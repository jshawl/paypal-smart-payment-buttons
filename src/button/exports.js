...
export function setupExports({
  ...
  const fundingSources = querySelectorAll(`[${DATA_ATTRIBUTES.FUNDING_SOURCE}]`)
    .map((el) => {
  ...
  window.exports = {
    ...
    intent,
    isGuestEnabled: () => {
      return fundingEligibility?.card?.hasOwnProperty("guestEnabled")
        ? fundingEligibility.card.guestEnabled
        : getGuestEnabledStatus(merchantID);
    },
    isShippingChangeEnabled: () => {
      return typeof onShippingChange === "function";
    },
    paymentSession: () => {
      return {
        ...
        upgradeFacilitatorAccessToken: ({
          facilitatorAccessToken: merchantAccessToken,
          orderID,
        }) => {
          const buyerAccessToken = getBuyerAccessToken();

          if (!buyerAccessToken) {
            getLogger().error("lsat_upgrade_error", {
              err: "buyer access token not found",
            });
            throw new Error("Buyer access token not found");
          }

          return upgradeFacilitatorAccessToken(merchantAccessToken, {
            buyerAccessToken,
            orderID,
          });
        },
