...
export function setupPaymentFlows({
  props,
  config,
  serviceData,
  components,
...
|}): PaymentFlow {
  ...
  for (const flow of PAYMENT_FLOWS) {
    if (
      flow.isEligible({ props, config, serviceData }) &&
...
const sendPersonalizationBeacons = (personalization) => {
  if (
    ...
    personalization.tagline.tracking
  ) {
...
export function initiatePaymentFlow({
  payment,
  serviceData,
  config,
  components,
  props,
  experiments,
}: InitiatePaymentOptions): ZalgoPromise<void> {
  ...
  return ZalgoPromise.try(() => {
    ...
    const { name, init, inline, spinner, updateFlowClientConfig } =
      getPaymentFlow({ props, payment, config, components, serviceData });
    const { click, start, close } = init({
      props,
      config,
      serviceData,
      components,
    ...
    getLogger()
      ...
      .info(
        `cross_site_tracking_${
          isCrossSiteTrackingEnabled("enforce_policy") ? "enabled" : "disabled"
        }`,
      )
      .track({
      ...
      .track({
        [FPTI_KEY.STATE]: FPTI_STATE.BUTTON,
        [FPTI_KEY.TRANSITION]: `cross_site_tracking_${
          isCrossSiteTrackingEnabled("enforce_policy") ? "enabled" : "disabled"
        }`,
      })
      .flush();
    ...
    const loggingPromise = ZalgoPromise.try(() => {
      return window.xprops.sessionState
        .get(`__confirm_${fundingSource}_payload__`)
        .then((confirmPayload) => {
          const fieldsSessionID = confirmPayload
            ? confirmPayload.payment_source[fundingSource].metadata
                .fieldsSessionID
            : "";
    ...
    return ZalgoPromise.try(() => {
      ...
      .then((valid) => {
        ...
        const startPromise = updateClientConfigPromise.then(() => {
          if (featureFlags.isButtonClientConfigCallBlocking) {
            getLogger().info("blocking_cco_call_resolved", {
              time: getClientsideTimestamp(),
              fundingSource,
              buttonSessionID,
            });
          } else {
            getLogger().info("non_blocking_cco_call_resolved", {
              time: getClientsideTimestamp(),
              fundingSource,
              buttonSessionID,
            });
        ...
        return ZalgoPromise.all([
          ...
          .then(() => {
            if (featureFlags.isButtonClientConfigCallBlocking) {
              getLogger().info("redirect_to_xorouter_blocking_cco", {
                time: getClientsideTimestamp(),
                fundingSource,
                buttonSessionID,
              });
            } else {
              getLogger().info("redirect_to_xorouter_non_blocking_cco", {
                time: getClientsideTimestamp(),
                fundingSource,
                buttonSessionID,
              });
...
export function initiateMenuFlow({
  payment,
  serviceData,
  config,
  components,
  props,
  experiments,
}: InitiateMenuOptions): ZalgoPromise<void> {
  return ZalgoPromise.try(() => {
    const { fundingSource, button } = payment;

    const { name, setupMenu } = getPaymentFlow({
      props,
      payment,
      config,
      components,
