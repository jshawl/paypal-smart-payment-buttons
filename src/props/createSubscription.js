...
import {
  createAccessToken,
  createSubscription as createSubcriptionID,
  reviseSubscription,
} from "../api";
...
export type XCreateSubscription = (
  ?XCreateSubscriptionDataType,
  ?XCreateSubscriptionActionsType,
) => ZalgoPromise<string>;

export function buildXCreateSubscriptionData({
  paymentSource,
}: {|
  paymentSource: $Values<typeof FUNDING> | null,
|}): XCreateSubscriptionDataType {
...
export function buildXCreateSubscriptionActions({
...
|}): XCreateSubscriptionActionsType {
  const create = (data) => {
    return createSubcriptionID(facilitatorAccessToken, data, {
      partnerAttributionID,
      merchantID,
      clientID,
    });
...
export function getCreateSubscription(
  {
    createSubscription,
    partnerAttributionID,
    merchantID,
    clientID,
    paymentSource,
  }: CreateSubscriptionXProps,
  { facilitatorAccessToken }: {| facilitatorAccessToken: string |},
): ?CreateSubscription {
  if (createSubscription) {
    ...
    return () => {
      return createSubscription(
        buildXCreateSubscriptionData({ paymentSource }),
        ...
        if (!subscriptionID || typeof subscriptionID !== "string") {
          throw new Error(
            `Expected an subscription id to be passed to createSubscription`,
          );
        }
