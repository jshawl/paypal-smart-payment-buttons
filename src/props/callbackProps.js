...
export function getCallbackProps({
  ...
  const createVaultSetupToken = getCreateVaultSetupToken({ createVaultSetupToken: inputCreateVaultSetupToken, paymentSource });
   ...
   const onShippingChange = getOnShippingChange({ onShippingChange: inputOnShippingChange, partnerAttributionID, experiments, featureFlags, clientID, paymentSource }, { facilitatorAccessToken, createOrder });
   ...
   const onAuth = getOnAuth({ facilitatorAccessToken, createOrder, createSubscription, featureFlags, experiments });
