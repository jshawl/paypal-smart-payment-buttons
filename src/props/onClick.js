...
export type XOnClick = (
  XOnClickDataType,
  XOnClickActionsType,
) => ZalgoPromise<boolean | void>;
...
export function buildXOnClickData({
  fundingSource,
}: {|
  fundingSource: $Values<typeof FUNDING>,
|}): XOnClickDataType {
...
|}): OnClick | void {
  ...
  return memoize(
    ({ fundingSource }: {| fundingSource: $Values<typeof FUNDING> |}) => {
      return onClick(
        buildXOnClickData({ fundingSource }),
        buildXOnClickActions(),
      ).then((valid) => {
