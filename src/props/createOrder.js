...
import { vaultApprovalSessionIdToOrderId } from "../api/vault";
...
export type XCreateOrder = (
  XCreateOrderDataType,
  XCreateOrderActionsType,
) => ZalgoPromise<string>;

export type CreateOrder = () => ZalgoPromise<string>;

export function buildXCreateOrderData({
  paymentSource,
}: {|
  paymentSource: $Values<typeof FUNDING> | null,
|}): XCreateOrderDataType {
...
export function buildPaymentActions({
  ...
  const create = (data) => {
    ...
    if (payment.intent && payment.intent !== expectedIntent) {
      throw new Error(
        `Unexpected intent: ${payment.intent} passed to order.create. Expected ${expectedIntent}`,
      );
    }
    ...
    payment.transactions = payment.transactions.map((transaction) => {
      if (
        ...
        throw new Error(
          `Unexpected currency: ${transaction.amount.currency} passed to order.create. Please ensure you are passing /sdk/js?${SDK_QUERY_KEYS.CURRENCY}=${transaction.amount.currency} in the paypal script tag.`,
        );
      }
    ...
    payment.redirect_urls.return_url =
      payment.redirect_urls.return_url || `${getDomain()}/checkoutnow/error`;
    payment.redirect_urls.cancel_url =
      payment.redirect_urls.cancel_url || `${getDomain()}/checkoutnow/error`;
...
export function buildXCreateOrderActions({
  facilitatorAccessToken,
  intent,
  currency,
  merchantID,
  partnerAttributionID,
  experiments,
}: OrderOptions): XCreateOrderActionsType {
...
export function getCreateOrder(
  ...
  const isVaultWithoutPurchase =
    flow === "vault_without_purchase" && createVaultSetupToken;

  const data = buildXCreateOrderData({ paymentSource });
  const actions = buildXCreateOrderActions({
    facilitatorAccessToken,
    intent,
    currency,
    merchantID,
