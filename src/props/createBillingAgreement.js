...
export type XCreateBillingAgreement = (
  ?XCreateBillingAgreementDataType,
  ?XCreateBillingAgreementActionsType,
) => ZalgoPromise<string>;

export function buildXCreateBillingAgreementData({
  paymentSource,
}: {|
  paymentSource: $Values<typeof FUNDING> | null,
|}): XCreateBillingAgreementDataType {
...
export function getCreateBillingAgreement({
  ...
  if (createBillingAgreement) {
    return () => {
      return createBillingAgreement(
        buildXCreateBillingAgreementData({ paymentSource }),
        buildXCreateBillingAgreementActions(),
      ).then((billingToken) => {
