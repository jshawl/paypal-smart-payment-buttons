...
export type XOnShippingOptionsChange = (
  XOnShippingOptionsChangeDataType,
  XOnShippingOptionsChangeActionsType,
) => ZalgoPromise<void>;
...
export function buildXOnShippingOptionsChangeActions({
  data,
  actions: passedActions,
  orderID,
}: {|
  data: OnShippingOptionsChangeData,
  actions: OnShippingOptionsChangeActionsType,
  orderID: string,
|}): XOnShippingOptionsChangeActionsType {
  const patchQueries = {};
  ...
  if (Object.keys(breakdown).length === 0) {
    throw new Error(
      "Must pass breakdown into data attribute for onShippingAddressChange callback.",
    );
  }
  ...
  const actions = {
    reject: passedActions.reject
      ? (message) => {
          if (
            Object.values(SHIPPING_OPTIONS_ERROR_MESSAGES).indexOf(message) ===
            -1
          ) {
...
export type OnShippingOptionsChange = (
  ...
  OnShippingOptionsChangeActionsType,
) => ZalgoPromise<void>;
...
export function getOnShippingOptionsChange(
  { onShippingOptionsChange }: OnShippingOptionsChangeXProps,
    ...
    return ({ ...data }, actions) => {
      return createOrder().then((orderID) => {
        getLogger()
          ...
          .track({
            [FPTI_KEY.TRANSITION]:
              FPTI_TRANSITION.CHECKOUT_SHIPPING_OPTIONS_CHANGE,
            [FPTI_KEY.EVENT_NAME]:
              FPTI_TRANSITION.CHECKOUT_SHIPPING_OPTIONS_CHANGE,
        ...
        return onShippingOptionsChange(
          buildXOnShippingOptionsChangeData(data),
          buildXOnShippingOptionsChangeActions({ data, actions, orderID }),
        );
      });
    };
  }
