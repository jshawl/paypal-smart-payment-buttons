...
export type XOnShippingAddressChangeActionsType = {|
  ...
  buildOrderPatchPayload: (
    args?: BuildOrderPatchPayloadArgs,
  ) => ZalgoPromise<$ReadOnlyArray<Query>>,
|};

export type XOnShippingAddressChange = (
  XOnShippingAddressChangeDataType,
  XOnShippingAddressChangeActionsType,
) => ZalgoPromise<void>;
...
export function buildXOnShippingAddressChangeActions({
  ...
  data: OnShippingAddressChangeData,
  actions: OnShippingAddressChangeActionsType,
  orderID: string,
|}): XOnShippingAddressChangeActionsType {
  const patchQueries = {};

  let breakdown = data.amount?.breakdown
    ? breakdownKeyChanges(data.amount.breakdown)
    : {};

  if (Object.keys(breakdown).length === 0) {
    throw new Error(
      "Must pass amount with breakdown into data attribute for onShippingAddressChange callback.",
    );
  }
  ...
  const actions = {
    reject: passedActions.reject
      ? (message) => {
          if (
            Object.values(SHIPPING_ADDRESS_ERROR_MESSAGES).indexOf(message) ===
            -1
          ) {
    ...
    buildOrderPatchPayload: ({
      ...
      const selectedShippingOptionAmount =
        selectedShippingOption?.amount?.value ?? "0.00";
      ...
      const newAmount = calculateTotalFromShippingBreakdownAmounts({
        breakdown,
        updatedAmounts,
      });
      ...
      return getShippingOrderInfo(orderID).then((sessionData) => {
        ...
        const shippingMethods =
          sessionData?.checkoutSession?.cart?.shippingMethods || [];
        ...
        if (hasShippingMethods) {
          queries = updateOperationForShippingOptions({
...
export type OnShippingAddressChange = (
  ...
  OnShippingAddressChangeActionsType,
) => ZalgoPromise<void>;
...
export function getOnShippingAddressChange(
  { onShippingAddressChange }: OnShippingAddressChangeXProps,
    ...
    return ({ ...data }, actions) => {
      return createOrder().then((orderID) => {
        getLogger()
          ...
          .track({
            [FPTI_KEY.TRANSITION]:
              FPTI_TRANSITION.CHECKOUT_SHIPPING_ADDRESS_CHANGE,
            [FPTI_KEY.EVENT_NAME]:
              FPTI_TRANSITION.CHECKOUT_SHIPPING_ADDRESS_CHANGE,
        ...
        return onShippingAddressChange(
          buildXOnShippingAddressChangeData(data),
          buildXOnShippingAddressChangeActions({ data, actions, orderID }),
        );
      });
    };
  }
