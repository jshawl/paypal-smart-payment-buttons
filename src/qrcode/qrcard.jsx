...
function QRCard({ svgString }: {| svgString: string |}): mixed {
  ...
  const onCloseClick = () => {
    if (state !== QRCODE_STATE.DEFAULT) {
      ...
      logger
        .info(`VenmoDesktopPay_qrcode_survey`)
        .track({
  ...
  const frontView = (
    <div id="front-view" className="card">
      <p id="fee-disclaimer">No fees no matter how you pay</p>
      <div id="instructions">
        <InstructionIcon stylingClass="instruction-icon" />
        <span>To pay, scan the QR code with your Venmo app</span>
      </div>
  ...
  const surveyElement = <Survey survey={survey} onCloseClick={onCloseClick} />;
  ...
  const escapePathFooter = displayEscapePath && (
    <p className="escape-path">
      Don&apos;t have the app? Pay with{" "}
      <span
        className="escape-path__link"
        onClick={() => handleClick(FUNDING.PAYPAL)}
      >
        PayPal
      </span>{" "}
      or{" "}
      <span
        className="escape-path__link"
        onClick={() => handleClick(FUNDING.CARD)}
      >
        Credit/Debit card
      </span>
    </p>
    ...
    <Fragment>
      ...
      <div id="view-boxes" className={state}>
        ...
        <div className="card" id="back-view">
          ...
          <div className="auth-message">Go to your Venmo app and authorize</div>
          <div className="success-message">Venmo account authorized</div>
        </div>
        {window.xprops.debug && (
          <button
            type="button"
...
export function renderQRCode({ svgString }: RenderQRCodeOptions) {
  logger = setupNativeQRLogger();
  render(<QRCard svgString={svgString} />, getBody());
}
