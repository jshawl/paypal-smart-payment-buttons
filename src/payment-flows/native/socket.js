...
const getNativeSocket = memoize(
  ({
    sessionUID,
    firebaseConfig,
    version,
  }: NativeSocketOptions): MessageSocket => {
    ...
    nativeSocket.onError((err) => {
      ...
      if (
        stringifiedError &&
        stringifiedError.toLowerCase().indexOf("permission_denied") === -1
      ) {
        getLogger()
          .error("native_socket_error", { err: stringifiedError })
          .track({
...
export function connectNative({
  ...
  sessionUID,
  callbacks,
}: ConnectNativeOptions): NativeConnection {
  const { onInit, onApprove, onCancel, onShippingChange, onError, onFallback } =
    callbacks;
  ...
  const onShippingChangeListener = socket.on(
    SOCKET_MESSAGE.ON_SHIPPING_CHANGE,
    onShippingChange,
  );
