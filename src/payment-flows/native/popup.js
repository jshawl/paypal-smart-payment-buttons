...
import {
  ...
  getNativePopupUrl,
  getNativeDomain,
  getNativePopupDomain,
  getNativeFallbackUrl,
} from "./url";
...
function isDetectedAppEligible({
  fundingSource,
  appDetect,
}: AppEligibleOptions): boolean {
  if (appDetect === null) {
    return true;
  }
  ...
  if (fundingSource === FUNDING.PAYPAL) {
    if (
      appDetect.installed &&
      versionIsEligible({ version: appDetect?.version })
    ) {
      return true;
    } else {
      return false;
...
function getEligibility({
  ...
  stickinessID,
  appDetect,
}: EligibilityOptions): ZalgoPromise<boolean> {
  const {
    createOrder,
    onShippingChange,
    vault,
    ...
    return createOrder().then((orderID) => {
      return getNativeEligibility({
        ...
        const ineligibilityReason =
          eligibility && eligibility[fundingSource]?.ineligibilityReason
        ...
        if (
          ...
          getLogger()
            .info(`native_appswitch_ineligible`, { orderID })
            .track({
              [FPTI_KEY.STATE]: FPTI_STATE.BUTTON,
              [FPTI_KEY.TRANSITION]:
                FPTI_TRANSITION.NATIVE_APP_SWITCH_INELIGIBLE,
...
export function initNativePopup({
  payment,
  props,
  serviceData,
  config,
  ...
  callbacks,
  clean,
}: NativePopupOptions): NativePopup {
  ...
  const getNativePopupProxyWindow = (url: string): ProxyWindow => {
    ...
    getLogger()
      .info(`native_attempt_appswitch_popup_shown`)
      .track({
        [FPTI_KEY.STATE]: FPTI_STATE.BUTTON,
    ...
    click: () => {
      nativePopupPromise = new ZalgoPromise((resolve, reject) => {
        ...
        const handleFallback = (
          fallbackOptions?: NativeFallbackOptions,
        ): ZalgoPromise<{| buttonSessionID: string |}> => {
        ...
        const changeDomainAndAwaitFallback = ({
          pageUrl,
          stickinessID,
          fallbackOptions,
        }: {|
        ...
        |}) => {
          return nativePopupWinProxy.isClosed().then((isClosed) => {
            ...
            return orderPromise.then((orderID) => {
              nativePopupWinProxy.setLocation(
                getNativeFallbackUrl({
                  props,
                  serviceData,
                  config,
        ...
        const onDetectPossibleAppSwitch = once(
          ({
            pageUrl,
            stickinessID,
          }: {|
          ...
          |}): ZalgoPromise<void> => {
            return ZalgoPromise.try(() => {
              ...
              getLogger()
                .info(`native_detect_possible_app_switch`)
                .track({
                  [FPTI_KEY.TRANSITION]:
                    FPTI_TRANSITION.NATIVE_DETECT_POSSIBLE_APP_SWITCH,
                })
                .flush();
        ...
        const closeListener = onCloseProxyWindow(
          nativePopupWinProxy,
          () => {
            getLogger()
              .info(`native_popup_closed`)
              .track({
                [FPTI_KEY.STATE]: FPTI_STATE.BUTTON,
        ...
        const closePopup = (event: string) => {
          getLogger()
            .info(`native_closing_popup_${event}`)
            .track({
              [FPTI_KEY.STATE]: FPTI_STATE.BUTTON,
              [FPTI_KEY.TRANSITION]: event
                ? `${FPTI_TRANSITION.NATIVE_CLOSING_POPUP}_${event}`
                : FPTI_TRANSITION.NATIVE_CLOSING_POPUP,
            })
            .flush();
        ...
        const awaitRedirectListener = postRobotOnceProxy(
          POST_MESSAGE.AWAIT_REDIRECT,
          { proxyWin: nativePopupWinProxy, domain: nativePopupDomain },
          ({ data: { app: appDetect, pageUrl, sfvc, stickinessID } }) => {
            ...
            const onDetectPossibleAppSwitchListener = postRobotOnceProxy(
              POST_MESSAGE.DETECT_POSSIBLE_APP_SWITCH,
              { proxyWin: nativePopupWinProxy, domain: nativePopupDomain },
              () => {
                getLogger()
                  .info(`native_post_message_detect_possible_app_switch`)
                  .flush();
                return onDetectPossibleAppSwitch({ pageUrl, stickinessID });
              },
            );

            const onDetectWebSwitchListener = postRobotOnceProxy(
              POST_MESSAGE.DETECT_WEB_SWITCH,
              {
                proxyWin: nativePopupWinProxy,
                domain: getNativeDomain({ props }),
              },
              () => {
                getLogger()
                  .info(`native_post_message_detect_web_switch`)
                  .flush();
                return onDetectWebSwitch();
              },
            );

            const onApproveListener = postRobotOnceProxy(
              POST_MESSAGE.ON_APPROVE,
              { proxyWin: nativePopupWinProxy, domain: nativePopupDomain },
              ({ data }) => {
                onDetectAppSwitch();
                onApprove({ data });
            ...
            const onCancelListener = postRobotOnceProxy(
              POST_MESSAGE.ON_CANCEL,
              { proxyWin: nativePopupWinProxy, domain: nativePopupDomain },
              () => {
                onDetectAppSwitch();
                onCancel();
            ...
            const onFallbackListener = postRobotOnceProxy(
              POST_MESSAGE.ON_FALLBACK,
              { proxyWin: nativePopupWinProxy, domain: nativePopupDomain },
              ({ data: fallbackOptions }) => {
                onDetectAppSwitch();
                getLogger()
                  .info(`native_message_onfallback`)
                  .track({
            ...
            const onCompleteListener = postRobotOnceProxy(
              POST_MESSAGE.ON_COMPLETE,
              { proxyWin: nativePopupWinProxy, domain: nativePopupDomain },
              () => {
                onDetectAppSwitch();
                getLogger()
                  .info(`native_post_message_on_complete`)
                  .track({
                    [FPTI_KEY.STATE]: FPTI_STATE.BUTTON,
            ...
            const onErrorListener = postRobotOnceProxy(
              POST_MESSAGE.ON_ERROR,
              { proxyWin: nativePopupWinProxy, domain: nativePopupDomain },
              ({ data }) => {
                onError({ data });
                closePopup("onError");
            ...
            return ZalgoPromise.hash({
                ...
                return orderPromise.then((orderID) => {
                  ...
                  getLogger()
                    ...
                    .track({
                      [FPTI_KEY.STATE]: FPTI_STATE.BUTTON,
                      [FPTI_KEY.TRANSITION]:
                        FPTI_TRANSITION.NATIVE_ATTEMPT_APP_SWITCH,
                  ...
                  if (isAndroidChrome()) {
                    closeListener.cancel();
                    const appSwitchCloseListener = onCloseProxyWindow(
                      nativePopupWinProxy,
                      () =>
                        onDetectPossibleAppSwitch({ pageUrl, stickinessID }),
                      50,
                    );
              ...
              .catch((err) => {
                getLogger()
                  .info(`native_attempt_appswitch_url_popup_errored`)
                  .track({
                    [FPTI_KEY.STATE]: FPTI_STATE.BUTTON,
                    [FPTI_KEY.TRANSITION]:
                      FPTI_TRANSITION.NATIVE_ATTEMPT_APP_SWITCH_ERRORED,
