...
export function getNativeDomain({ props }: {| props: ButtonProps |}): string {
  ...
  if (
    env === ENV.SANDBOX &&
    isNativeOptedIn({ props }) &&
    !(window.xprops && window.xprops.useCorrectNativeSandboxDomain)
  ) {
...
export function getNativePopupDomain({
  props,
}: {|
  props: ButtonProps,
|}): string {
  ...
  const isMobileNativePopupDomainEnabled = createExperiment(
    "enable_mobile_native_popup_domain",
    experimentOptions,
  ).isEnabled();
...
export function getWebCheckoutUrl({
  orderID,
  props,
  fundingSource,
  facilitatorAccessToken,
}: GetWebCheckoutUrlOptions): string {
  const { commit } = props;
...
function getNativeUrlQueryParams({
  props,
  serviceData,
  config,
  fundingSource,
  ...
  orderID,
  stickinessID,
}: GetNativeUrlOptions): NativeUrlQuery {
  const {
    env,
    clientID,
    commit,
  ...
  const webCheckoutUrl = getWebCheckoutUrl({
    orderID,
    props,
    fundingSource,
    facilitatorAccessToken,
...
export function getNativeUrl({
  ...
  orderID,
  stickinessID,
}: GetNativeUrlOptions): string {
  const queryParams = getNativeUrlQueryParams({
    props,
    serviceData,
    config,
    fundingSource,
  ...
  return extendUrl(
    `${getNativeDomain({ props })}${NATIVE_CHECKOUT_URI[fundingSource]}`,
    {
      // $FlowFixMe
      query: queryParams,
    },
...
export function getNativeFallbackUrl({
  props,
  serviceData,
  config,
  fundingSource,
...
const getNativePopupQueryParams = ({
  props,
  serviceData,
}: GetNativePopupUrlOptions): NativePopupInputParams => {
...
export function getNativePopupUrl({
  props,
  serviceData,
  fundingSource,
}: GetNativePopupUrlOptions): string {
  const queryParams = getNativePopupQueryParams({
    props,
    serviceData,
    fundingSource,
  });

  const baseURL = extendUrl(
    `${getNativePopupDomain({ props })}${
