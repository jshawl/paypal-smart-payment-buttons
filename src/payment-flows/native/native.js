...
import {
  isNativeEligible,
  isNativePaymentEligible,
  prefetchNativeEligibility,
  canUsePopupAppSwitch,
  canUseNativeQRCode,
  canUseVenmoWeb,
  setNativeOptOut,
  getDefaultNativeFallbackOptions,
...
function setupNative({
  ...
  props,
  serviceData,
}: SetupOptions): ZalgoPromise<void> {
...
function initNative({
  ...
  const { onApprove, onCancel, onError, buttonSessionID, onShippingChange } =
    props;
  const {
    eligibility: { venmoWebEnabled },
  } = serviceData;
  ...
  const fallbackToWebCheckout = (
    fallbackWin?: ?(CrossDomainWindowType | ProxyWindow),
  ): ZalgoPromise<void> => {
    didFallback = true;
    ...
    return ZalgoPromise.try(() => {
      ...
      return ZalgoPromise.all([destroy(), instance.start()]).then(noop);
    });
  };
  ...
  const onApproveCallback = ({
    data: { payerID, paymentID, billingToken },
  }) => {
    approved = true;

    getLogger()
      .info(`native_message_onapprove`, { payerID, paymentID, billingToken })
      .track({
        [FPTI_KEY.TRANSITION]: FPTI_TRANSITION.NATIVE_ON_APPROVE,
        [FPTI_CUSTOM_KEY.INFO_MSG]: `payerID: ${payerID}, paymentID: ${
          paymentID || "undefined"
        }, billingToken: ${billingToken || "undefined"}`,
      })
      .flush();
    ...
    return ZalgoPromise.all([
      onApprove(data, actions).catch((err) => {
        getLogger()
          .info(`native_message_onapprove_error`, {
            payerID,
            paymentID,
            billingToken,
          })
  ...
  const onCancelCallback = () => {
    cancelled = true;
    getLogger()
      .info(`native_message_oncancel`)
      .track({
        [FPTI_KEY.TRANSITION]: FPTI_TRANSITION.NATIVE_ON_CANCEL,
      })
      .flush();

    return ZalgoPromise.all([onCancel(), destroy()]).then(() => {
      return { buttonSessionID };
    });
  };
  ...
  |}) => {
    getLogger()
      .info(`native_message_onerror`, { err: message })
      .track({
    ...
    return ZalgoPromise.all([onError(new Error(message)), destroy()]).then(
      () => {
        return { buttonSessionID };
      },
    );
  ...
  const onShippingChangeCallback = ({
    data,
  }: {|
  ...
  |}) => {
    return ZalgoPromise.try(() => {
      getLogger()
        .info(`native_message_onshippingchange`)
        .track({
  ...
  const onCloseCallback = () => {
    return ZalgoPromise.delay(1000)
      .then(() => {
  ...
  |}) => {
    const {
      win: fallbackWin,
      fallbackOptions = getDefaultNativeFallbackOptions(),
    } = opts || {};
  ...
  if (
    canUseVenmoWeb({ fundingSource, win, serviceData }) &&
  ...
  const start = () => {
    return ZalgoPromise.try(() => {
      ...
      return destroy().then(() => {
        getLogger()
          .error(`native_error`, { err: stringifyError(err) })
          .track({
...
function updateNativeClientConfig({
  orderID,
  payment,
  userExperienceFlow,
  buttonSessionID,
