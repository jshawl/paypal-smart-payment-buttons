...
function isPaymentFieldsEligible({
  props,
  serviceData,
  ...
  if (
    componentsList.includes("marks") &&
    componentsList.includes("payment-fields")
  ) {
    return false;
  }
...
function isPaymentFieldsPaymentEligible({
  payment,
  serviceData,
...
function highlightFundingSource(fundingSource: ?$Values<typeof FUNDING>) {
  ...
  querySelectorAll(`[${DATA_ATTRIBUTES.FUNDING_SOURCE}]`).forEach((el) => {
    if (
      el.getAttribute(DATA_ATTRIBUTES.FUNDING_SOURCE) ===
...
const getElements = (
  ...
  fundingSourceButtonsContainer: HTMLElement,
  paymentFieldsContainer: HTMLElement,
|} => {
  const buttonsContainer = document.querySelector("#buttons-container");
  let fundingSourceButtonsContainer;
  if (fundingSource) {
    fundingSourceButtonsContainer = document.querySelector(
      `[${DATA_ATTRIBUTES.FUNDING_SOURCE}="${fundingSource}"]`,
    );
  }
  const paymentFieldsContainer = document.querySelector(
    "#payment-fields-container",
  );

  if (
    !buttonsContainer ||
    !fundingSourceButtonsContainer ||
...
const slideUpButtons = (fundingSource: ?$Values<typeof FUNDING>) => {
  ...
  const recalculateMargin = () => {
    buttonsContainer.style.marginTop = `${
      buttonsContainer.offsetTop - fundingSourceButtonsContainer.offsetTop
    }px`;
    paymentFieldsContainer.style.marginTop = `${
      buttonsContainer.offsetTop -
      (fundingSourceButtonsContainer.offsetTop +
        fundingSourceButtonsContainer.offsetHeight)
    }px`;
  };
...
const slideDownButtons = (fundingSource: ?$Values<typeof FUNDING>) => {
  const { buttonsContainer, paymentFieldsContainer } =
    getElements(fundingSource);
    ...
    onContinue: (data: ConfirmData) => {
      return createOrder().then((orderID) => {
        return getConfirmOrder(
          ...
          .then(() => {
            ...
            instance = Checkout({
              ...
              onApprove: ({ payerID, paymentID, billingToken }) => {
                ...
                return close().then(() => {
                  return onApprove(
                    { payerID, paymentID, billingToken, buyerAccessToken },
                    { restart },
                  ).catch(noop);
                });
              },
          ...
          .catch((err) => {
            getLogger().error("payment_fields_confirm_order_error", {
