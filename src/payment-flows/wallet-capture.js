...
function isWalletCaptureEligible({
  props,
  serviceData,
...
function setupWalletCapture({ props, config, serviceData }: SetupOptions) {
  ...
  const headers = disableSetCookie
...
function isWalletCapturePaymentEligible({
  serviceData,
  payment,
...
function initWalletCapture({
  ...
  const createAccessToken = () => {
    ...
    return smartWalletPromise.then((smartWallet) => {
      const { accessToken } = getInstrument(
        smartWallet,
        fundingSource,
        instrumentID,
      );
  ...
  const getWebCheckoutFallback = () => {
    return checkout.init({
      ...
      payment: {
        ...
        fundingSource:
          instrument && instrument.type === WALLET_INSTRUMENT.CREDIT
            ? FUNDING.CREDIT
            : fundingSource,
      },
      config,
  ...
  const start = () => {
    return ZalgoPromise.hash({
      ...
      .then(({ orderID, smartWallet }) => {
        const buyerAccessToken = getInstrument(
          smartWallet,
          fundingSource,
          instrumentID,
        ).accessToken;
        ...
        return ZalgoPromise.hash({
          ...
          return onApprove({ payerID, buyerAccessToken }, { restart }).catch(
            noop,
          );
        });
      })
      .catch((err) => {
        getLogger()
          .warn("approve_order_error", { err: stringifyError(err) })
          .flush();
        return fallbackToWebCheckout();
      });
  };
...
function setupWalletMenu({
  ...
  const newFundingSource =
    instrument.type === WALLET_INSTRUMENT.CREDIT
      ? FUNDING.CREDIT
      : fundingSource;
      ...
      return ZalgoPromise.try(() => {
        ...
        return loadCheckout({
          payment: {
            ...
            createAccessToken: () => {
              return smartWalletPromise.then((smartWallet) => {
                const smartInstrument = getInstrument(
                  smartWallet,
                  fundingSource,
                  instrumentID,
                );
...
function updateWalletClientConfig({
  orderID,
  payment,
  featureFlags,
}): ZalgoPromise<void> {
