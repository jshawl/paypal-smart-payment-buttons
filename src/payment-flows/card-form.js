...
function isCardFormPaymentEligible({
  payment,
...
function highlightCard(card: ?$Values<typeof CARD>) {
  ...
  querySelectorAll(`[${DATA_ATTRIBUTES.CARD}]`).forEach((el) => {
    el.style.opacity =
      el.getAttribute(DATA_ATTRIBUTES.CARD) === card ? "1" : "0.1";
  });
}
...
const getElements = (): {|
  ...
  cardButtonsContainer: HTMLElement,
  cardFormContainer: HTMLElement,
|} => {
  const buttonsContainer = document.querySelector("#buttons-container");
  const cardButtonsContainer = document.querySelector(
    `[${DATA_ATTRIBUTES.FUNDING_SOURCE}="${FUNDING.CARD}"]`,
  );
...
const slideUpButtons = () => {
  const { buttonsContainer, cardButtonsContainer, cardFormContainer } =
    getElements();
  ...
  const recalculateMargin = () => {
    buttonsContainer.style.marginTop = `${
      buttonsContainer.offsetTop - cardButtonsContainer.offsetTop
    }px`;
  };
