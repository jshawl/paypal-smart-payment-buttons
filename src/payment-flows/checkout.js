...
import {
  exchangeAccessTokenForAuthCode,
  getConnectURL,
...
function isConnectEligible({
  connect,
  vault,
  fundingSource,
  createBillingAgreement,
  createSubscription,
}: ConnectEligibleOptions): boolean {
  if (!connect) {
    return false;
  }
...
export const getDimensions = (
  ...
  if (isWebView()) {
    ...
    return {
      width: CHECKOUT_APM_POPUP_DIMENSIONS.WIDTH,
      height: CHECKOUT_APM_POPUP_DIMENSIONS.HEIGHT,
    };
...
function initCheckout({
  ...
  const connectEligible = isConnectEligible({
    connect,
    createBillingAgreement,
    createSubscription,
    vault,
  ...
  const init = () => {
    return Checkout({
      ...
      createAuthCode: () => {
        return ZalgoPromise.try(() => {
          ...
          if (payment.createAccessToken) {
            ...
            checkoutRestart &&
            buyerAccessTokenReceivedOnAuth &&
            fundingSkipLogin
          ) {
            ...
            const queryStringParams = disableSetCookie
              ? { disableSetCookie }
              : {};
            return loadFraudnet({
              env,
              ...
              .then(() => {
                const headers = {
                  [HEADERS.DISABLE_SET_COOKIE]: String(disableSetCookie),
                };
          ...
          .then((accessToken) => {
            if (
              accessToken &&
              (buyerIntent === BUYER_INTENT.PAY ||
                buyerIntent ===
                  BUYER_INTENT.PAY_WITH_DIFFERENT_FUNDING_SHIPPING)
            ) {
      ...
      dimensions: getDimensions(
        fundingSource,
        experiments?.popupIncreaseDimensions ?? false,
      ),
      ...
      if (!merchantRequestedPopupsDisabled && !win && supportsPopups()) {
        try {
          ...
          if (err instanceof PopupOpenError) {
            ...
            getLogger().warn("popup_open_error_blocked", {
              err: stringifyError(err),
            });
            sendCountMetric({
...
function updateCheckoutClientConfig({
  orderID,
  payment,
  userExperienceFlow,
  featureFlags,
  ...
  return ZalgoPromise.try(() => {
    ...
    const updateClientConfigPromise = updateButtonClientConfig({
      fundingSource,
      orderID,
      inline: false,
      userExperienceFlow,
    ...
    const isButtonClientConfigCallBlocking =
      featureFlags && featureFlags.isButtonClientConfigCallBlocking;
    // Block
    if (
      buyerIntent === BUYER_INTENT.PAY_WITH_DIFFERENT_FUNDING_SHIPPING ||
      isButtonClientConfigCallBlocking
    ) {
