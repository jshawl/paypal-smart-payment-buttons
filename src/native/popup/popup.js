...
export function setupNativePopup({
  parentDomain,
  env,
  sessionID,
  buttonSessionID,
  ...
  const logger = setupNativeLogger({
    env,
    sessionID,
    buttonSessionID,
    sdkCorrelationID,
  ...
  if (venmoWebEnabled && venmoWebUrl) {
    logger
      .info("native_venmo_web_redirect", {
        buttonSessionID,
      })
      .track({
  ...
  const appInstalledPromise = isAppInstalled({ fundingSource, env }).catch(
    (err) => {
      logger
        .info("native_popup_android_app_installed_error")
        .track({
  ...
  window.addEventListener("beforeunload", () => {
    logger
      .info("native_popup_beforeunload")
      .track({
  ...
  if (!opener) {
    if (isIOSSafari()) {
      const log = `${
        FPTI_TRANSITION.NATIVE_POPUP_NO_OPENER
      }_hash_${getRawHash()}_${logMessage}`;
      logger
        .info(log)
        .track({
          [FPTI_KEY.TRANSITION]: log,
    ...
    logger
      ...
      .track({
        ...
        [FPTI_CUSTOM_KEY.INFO_MSG]: `location: ${base64encode(
    ...
    onCloseWindow(
      ...
      () => {
        logger
          .info(`native_popup_opener_detect_close`)
          .track({
            [FPTI_KEY.TRANSITION]:
              FPTI_TRANSITION.NATIVE_POPUP_OPENER_DETECT_CLOSE,
          })
          .flush()
          .then(closeWindow);
      },
  ...
  const sendToParent = (event, payload = {}) => {
    return postRobot
      .send(opener, event, payload, { domain: parentDomain })
      .then(({ data }) => data);
  };
  ...
  const handleHash = () => {
    ...
    switch (hash) {
      ...
      case HASH.ON_APPROVE: {
        ...
        sendToParent(MESSAGE.ON_APPROVE, {
          payerID,
          paymentID,
          billingToken,
        }).finally(closeWindow);
      ...
      case HASH.ON_FALLBACK: {
        const { type, skip_native_duration, fallback_reason } =
          parseQuery(queryString);
        sendToParent(MESSAGE.ON_FALLBACK, {
          type,
          skip_native_duration,
          fallback_reason,
        });
        break;
      }
  ...
  clean.register(() =>
    window.removeEventListener(EVENT.HASHCHANGE, handleHash),
  );
  ...
  appInstalledPromise.then((app) => {
    ...
    sendToParent(MESSAGE.AWAIT_REDIRECT, {
      app,
      pageUrl,
      sfvc,
      stickinessID,
    }).then(({ redirect = true, redirectUrl, orderID, appSwitch = true }) => {
      if (!redirect) {
        return;
      }
      ...
      clean.register(() =>
        window.removeEventListener("pagehide", markRedirect),
      );
