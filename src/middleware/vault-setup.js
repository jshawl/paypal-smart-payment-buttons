...
function isFundingSourceVaultable({
  accessToken,
  fundingSource,
  clientID,
  merchantID,
  ...
  disableFunding,
  disableCard,
}: IsFundingSourceVaultableOptions): ZalgoPromise<boolean> {
  return ZalgoPromise.try(() => {
    ...
    return getFundingEligibility(
      ...
      if (
        newFundingEligibility[fundingSource] &&
        newFundingEligibility[fundingSource].vaultable
      ) {
        return true;
      } else {
        return false;
...
function isVaultAutoSetupEligible({
  vault,
  clientAccessToken,
  ...
  disableCard,
  userIDToken,
}: VaultAutoSetupEligibleProps): ZalgoPromise<boolean> {
  return ZalgoPromise.try(() => {
    ...
    return isFundingSourceVaultable({
...
export function enableVaultSetup({
  orderID,
  vault,
  clientAccessToken,
  createBillingAgreement,
  ...
  userExperienceFlow,
  buttonSessionID,
}: EnableVaultSetupOptions): ZalgoPromise<void> {
  return ZalgoPromise.try(() => {
    getLogger()
      .info(
        `vault_auto_setup_vault_${vault.toString()}_id_token_${
          userIDToken ? "present" : "not_present"
        }`,
      )
      .flush();

    return isVaultAutoSetupEligible({
      vault,
      clientAccessToken,
      createBillingAgreement,
      createSubscription,
