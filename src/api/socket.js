...
export function messageSocket({
  ...
  targetApp,
  retry = true,
}: MessageSocketOptions): MessageSocket {
  ...
  const sendResponse = (
    socket,
    { messageName, responseStatus, responseData, requestUID },
  ) => {
    if (!socket.isOpen()) {
      return;
    }
  ...
  const onRequest = (
    socket,
    { messageSessionUID, requestUID, messageName, messageData },
  ) => {
    ...
    return ZalgoPromise.try(() => {
      ...
      if (requireSessionUID && messageSessionUID !== sessionUID) {
        throw new Error(
          `Incorrect sessionUID: ${messageSessionUID || "undefined"}`,
        );
      }
  ...
  const onResponse = ({
    ...
    const { listenerPromise, requireSessionUID } =
      responseListeners[requestUID] || {};

    if (!listenerPromise) {
      return triggerError(
        new Error(
          `Could not find response listener for ${messageName} with id: ${requestUID}`,
        ),
      );
    }
    ...
    if (requireSessionUID && messageSessionUID !== sessionUID) {
      return triggerError(
        new Error(`Incorrect sessionUID: ${messageSessionUID || "undefined"}`),
      );
    }
    ...
    if (responseStatus === RESPONSE_STATUS.SUCCESS) {
      ...
      listenerPromise.reject(
        new Error(`Can not handle response status: ${status || "undefined"}`),
      );
    }
  };
  ...
  const send = <T, R>(
    messageName,
    messageData: T,
    { requireSessionUID = true, timeout = 0 } = {},
  ): ZalgoPromise<R> => {
    return socketPromise.then((socket) => {
      ...
      if (timeout) {
        setTimeout(() => {
          listenerPromise.reject(
            new Error(
              `Timeoued out waiting for ${messageName} response after ${timeout}ms`,
            ),
          );
        }, timeout);
      }
  ...
  const close = () => {
    ...
    ZalgoPromise.all(activeRequests).then(() => {
      return socketPromise.then((socket) => socket.close(), noop);
    });
  };
...
export function webSocket({
  ...
  sourceAppVersion,
  targetApp,
}: WebSocketOptions): MessageSocket {
  const driver = () => {
...
export function firebaseSocket({
  sessionUID,
  config,
  sourceApp,
  sourceAppVersion,
  targetApp,
}: FirebaseSocketOptions): MessageSocket {
  const driver = () => {
    let open = false;
    ...
    const databasePromise = ZalgoPromise.hash({
      ...
      return firebase
        ...
        .then(() => {
          ...
          getLogger()
            .info("firebase_connection_opened")
            .track({
          ...
          database
            .ref(`users/${sessionUID}/messages`)
            .on("value", valueCallback, (err) => {
              error(err);
            });
    ...
    databasePromise.catch((err) => {
      getLogger()
        .warn("firebase_connection_errored", { err: stringifyError(err) })
        .track({
          [FPTI_KEY.STATE]: FPTI_STATE.BUTTON,
    ...
    return {
      send: (data) => {
        databasePromise
          .then((database) => {
            return database
              .ref(`users/${sessionUID}/messages/${uniqueID()}`)
              .set(data);
          })
          .catch(error);
      },
