...
function createRequest(
  accessToken: string,
  subscriptionPayload: SubscriptionCreateRequest,
  partnerAttributionID?: ?string,
  eventName: string,
): ZalgoPromise<string> {
...
export function createSubscription(
  accessToken: string,
  subscriptionPayload: SubscriptionCreateRequest,
  { partnerAttributionID, merchantID, clientID }: SubscriptionOptions,
): ZalgoPromise<string> {
  ...
  if (merchantID && merchantID[0]) {
    ...
    return createAccessToken(clientID, { targetSubject: merchantID[0] }).then(
      (thirdPartyAccessToken): ZalgoPromise<string> => {
...
function reviseRequest(
  ...
  return request({
    ...
    if (status !== 200) {
      getLogger().warn(`rest_api_${eventName}_error`);
      throw new Error(
        `Revise Subscription Api HTTP-${status} response: error:\n\n${JSON.stringify(
          body,
          null,
          4,
        )}`,
...
export function reviseSubscription(
  ...
  if (!subscriptionID) {
    throw new Error(
      `Expected subscription id to be passed as first argument to revise subscription api`,
    );
  }
...
export function activateSubscription(
  subscriptionID: string,
  { buyerAccessToken }: SubscriptionAPICredentials,
): ZalgoPromise<SubscriptionResponse> {
  return callSmartAPI({
