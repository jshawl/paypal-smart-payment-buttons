...
export function createPayment(
  ...
  { facilitatorAccessToken, partnerAttributionID }: PaymentAPIOptions,
): ZalgoPromise<PaymentResponse> {
  ...
  return createPayment(payment, {
    ...
    if (res.links && res.links.length) {
      for (let i = 0; i < res.links.length; i++) {
        if (
          res.links[i].method === "REDIRECT" &&
          res.links[i].rel === "approval_url"
        ) {
...
export function approveApplePayPayment(
  orderID: string,
  clientID: string,
  ...
  return callGraphQL({
    ...
    if (!gqlResult || !gqlResult.approveApplePayPayment) {
      throw new Error(
        `GraphQL GetApplePayPayment returned no applePayment object`,
      );
    }
