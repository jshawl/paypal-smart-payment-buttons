...
function handleThreeDomainSecureRedirect({
  ThreeDomainSecure,
  vaultToken,
  createOrder,
  action,
  getParent,
}: ThreeDomainSecureProps): ZalgoPromise<void> {
  ...
  return instance
    .renderTo(getParent(), TARGET_ELEMENT.BODY)
    .then(() => promise)
...
export function handleThreeDomainSecureContingency({
  status,
  links,
  ThreeDomainSecure,
  createOrder,
  ...
  paymentFlow,
  fundingSource,
}: ThreeDomainSecureContingencyProps): ZalgoPromise<void> | void {
  const isWithPurchase = (link) =>
    link.rel === "payer-action" && link.href && link.href.includes("flow=3ds");
  const isWithoutPurchase = (link) =>
    link.rel === "approve" && link.href.includes("helios");
...
export function handleValidatePaymentMethodResponse({
  ThreeDomainSecure,
  status,
  body,
  createOrder,
  getParent,
}: HandleValidatePaymentMethodResponse): ZalgoPromise<void> {
  return ZalgoPromise.try(() => {
    if (
      status === 422 &&
      body.links &&
      body.links.some((link) => link.rel === "3ds-contingency-resolution")
    ) {
      sendCountMetric({
        name: "pp.app.paypal_sdk.buttons.vault_capture.contingency.count",
        dimensions: {
