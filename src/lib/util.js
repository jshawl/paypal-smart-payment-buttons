...
export function promiseOne<T>(
  promises: $ReadOnlyArray<ZalgoPromise<T>>,
): ZalgoPromise<T> {
  return new ZalgoPromise((resolve, reject) => {
...
export function createExperiment(
  name: string,
  { sample, sticky = true }: CreateExperimentOptions,
): Experiment {
  ...
  return experiment({
    ...
    logCheckpoint({ treatment, checkpoint, payload, throttle }) {
      ...
      logger.info(`${treatment}_${checkpoint}`, {
        ...payload,
...
export function onCloseProxyWindow(
...
export function getNavigationTimeOrigin(): number {
  if (window.performance) {
    const hrSyncPoint = window.performance.now();
