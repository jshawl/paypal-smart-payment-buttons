  ...
  it("should render a button, click the button, and render checkout, then pass onApprove callback to the parent with actions.order.get and use RestAPI", async () => {
    return await wrapPromise(async ({ expect }) => {
      ...
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", async (data, actions) => {
  ...
  it("should render a button, click the button, and render checkout, then pass onShippingChange callback to the parent with actions.order.patch", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      window.xprops.onShippingChange = mockAsyncProp(
        expect("onShippingChange", async (data, actions) => {
    ...
    return await wrapPromise(async ({ expect }) => {
      ...
      window.xprops.createSubscription = mockAsyncProp(
        expect("createSubscription", async (data, actions) => {
          ...
          if (!subscriptionID) {
            throw new Error(
              `Expected subscriptionID to be returned by actions.subscription.create`,
            );
          }
      ...
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", async (data, actions) => {
          if (data.subscriptionID !== subscriptionID) {
            throw new Error(
              `Expected subscriptionID to be ${subscriptionID}, got ${data.subscriptionID}`,
            );
          }

          const getSubscriptionApiMock = getGetSubscriptionApiMock(
            {},
            mockSubscriptionID,
          );
          ...
          if (response.id !== subscriptionID) {
            throw new Error(
              `Expected subscriptionID to be ${subscriptionID}, got ${response.id} in Get Subscriptions response`,
            );
          }
          getSubscriptionApiMock.done();
          const activateSubscriptionIdApiMock =
            getActivateSubscriptionIdApiMock({}, mockSubscriptionID);
          ...
          if (JSON.stringify(activateResponse) !== "{}") {
            throw new Error(
              `Expected activate response to be 204 NO CONTENT , got ${JSON.stringify(
                activateResponse,
              )} in Activate Subscriptions`,
            );
          }
        ...
        expect("Checkout", ({ original: CheckoutOriginal, args: [props] }) => {
          ...
          mockFunction(
            ...
            expect("renderTo", async ({ original: renderToOriginal, args }) => {
              ...
              if (id !== mockCartID) {
                throw new Error(
                  `Expected orderID to be ${subscriptionID}, got ${id}`,
                );
              }
              return renderToOriginal(...args);
            }),
      ...
      const subscriptionIdToCartIdApiMock = getSubscriptionIdToCartIdApiMock(
        {},
        mockSubscriptionID,
        mockCartID,
      );
    ...
    return await wrapPromise(async ({ expect }) => {
      ...
      window.xprops.createSubscription = mockAsyncProp(
        expect("createSubscription", async (data, actions) => {
          ...
          subscriptionID = await actions.subscription.revise(
            mockSubscriptionID,
            {
          ...
          if (!subscriptionID) {
            throw new Error(
              `Expected subscriptionID to be returned by actions.subscription.revise`,
            );
          }
