...
describe("prerender cases", () => {
  it("should prerender a button, and call createOrder or onApprove", async () => {
    return await wrapPromise(async ({ expect }) => {
        ...
        expect(
          ...
          ({ original: CheckoutOriginal, args: [checkoutProps] }) => {
            if (checkoutProps.window.getWindow() !== win.getWindow()) {
              throw new Error(
                `Expected win passed to checkout to match win sent in prerender`,
              );
            }
      ...
      window.xprops.getPrerenderDetails = mockAsyncProp(
        expect("getPrerenderDetails", () => {
      ...
      window.xprops.createOrder = mockAsyncProp(
        expect("createOrder", async () => ZalgoPromise.resolve(orderID)),
      );
