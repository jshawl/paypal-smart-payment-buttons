...
import {
  $mockEndpoint,
  patchXmlHttpRequest,
} from "@krakenjs/sync-browser-mocks/src/xhr";
import {
  mockWebSocket,
  patchWebSocket,
} from "@krakenjs/sync-browser-mocks/src/webSocket";
...
export function mockAsyncProp(
  ...
  const asyncHandler = (...args) => {
    return ZalgoPromise.delay(time)
      .then(() => handler(...args))
      .then(
        (res) => {
          ZalgoPromise.delay(time)
            .then(() => currentPromise.resolve(res))
            .catch(noop);
          return res;
        },
        (err) => {
          ZalgoPromise.delay(time)
            .then(() => currentPromise.reject(err))
            .catch(noop);
          throw err;
        },
      );
...
export function setupMocks() {
          ...
          return ZalgoPromise.try(() => {
            if (props.window) {
              return ProxyWindow.toProxyWindow(props.window, {
                send: () => {
                  throw new Error(
                    `Can not send post message for proxy window in test`,
                  );
                },
              })
                .awaitWindow()
  ...
  const singleCardFieldContainer =
    document.querySelector("#card-fields-container") ||
    document.createElement("div");
  ...
  const paymentFieldsContainer =
    document.querySelector("#payment-fields-container") ||
    document.createElement("div");
...
export function mockFunction<T, A>(
  obj: mixed,
  prop: string,
  mock: ({|
    args: $ReadOnlyArray<A>,
    original: (...args: $ReadOnlyArray<A>) => T,
  |}) => T,
): {| cancel: () => void |} {
  // $FlowFixMe
...
export async function clickButton(
  fundingSource?: string = FUNDING.PAYPAL,
): Promise<void> {
...
export async function clickMenu(
  ...
  const menubutton = window.document
    .querySelector(`[data-funding-source=${fundingSource}]`)
    .parentElement.querySelector(`[data-menu]`);
...
export function enterButton(fundingSource?: string = FUNDING.PAYPAL) {
  triggerKeyPress(
    window.document.querySelector(`[data-funding-source=${fundingSource}]`),
    13,
  );
}
...
export function createButtonHTML({
  ...
  wallet,
}: {| fundingEligibility?: Object, wallet?: Object |} = {}) {
  const buttons = [];
  ...
  for (const fundingSource of values(FUNDING)) {
    ...
    if (fundingSource === FUNDING.CARD) {
      for (const card of values(CARD)) {
        ...
        if (
          cardConfig.vaultedInstruments &&
          cardConfig.vaultedInstruments.length
        ) {
          const vaultedInstrument = cardConfig.vaultedInstruments[0];
          buttons.push(
            `<div><div role="button" data-funding-source="${fundingSource}" data-payment-method-id="${vaultedInstrument.id}"></div><div data-menu></div></div>`,
          );
        } else {
          buttons.push(
            `<div><div role="button" data-funding-source="${fundingSource}"></div></div>`,
          );
        }
      }
    } else if (
      ...
      wallet[fundingSource].instruments.length
    ) {
      const walletInstrument = wallet[fundingSource].instruments[0];
      buttons.push(
        `<div><div role="button" data-funding-source="${fundingSource}" data-payment-method-id="${walletInstrument.tokenID}"></div>`,
      );
    } else {
      if (
        fundingConfig.vaultedInstruments &&
        fundingConfig.vaultedInstruments.length
      ) {
        ...
        if (walletInstrument.instrumentID) {
          buttons.push(
            `<div><div role="button" data-funding-source="${fundingSource}" data-instrument-id="${walletInstrument.instrumentID}"></div><div data-menu></div></div>`,
          );
        } else if (walletInstrument.paymentID) {
          buttons.push(
            `<div><div role="button" data-funding-source="${fundingSource}" data-payment-method-id="${walletInstrument.paymentID}"></div><div data-menu></div></div>`,
          );
        }
      } else {
        buttons.push(
...
export function getRestfulCaptureOrderApiMock(
  options: Object = {},
): MockEndpoint {
  return $mockEndpoint.register({
    method: "POST",
...
export function getRestfulAuthorizeOrderApiMock(
  options: Object = {},
): MockEndpoint {
  return $mockEndpoint.register({
    method: "POST",
...
export function getRestfulPatchOrderApiMock(
  options: Object = {},
): MockEndpoint {
  return $mockEndpoint.register({
    method: "PATCH",
...
export function getRestfulAuthorizationsCaptureApiMock(
  options: Object = {},
): MockEndpoint {
  return $mockEndpoint.register({
    method: "POST",
...
export function getSubscriptionIdToCartIdApiMock(
  options: Object = {},
  subscriptionID: string = "I-SUBSCRIPTIONID",
  cartId: string = "CARTIDOFSUBSCRIPTIONS",
): MockEndpoint {
  return $mockEndpoint.register({
    method: "POST",
...
export function getCreateSubscriptionIdApiMock(
  options: Object,
...
export function getReviseSubscriptionIdApiMock(
  options: Object,
  subscriptionID: string,
): MockEndpoint {
  return $mockEndpoint.register({
  ...
  return $mockEndpoint.register({
    ...
    handler: ({ uri, method, query, data, headers }) => {
      if (options.extraHandler) {
        const result = options.extraHandler({
          uri,
          method,
          query,
          data,
...
export function getValidatePaymentMethodApiMock(
  options: Object = {},
): MockEndpoint {
  return $mockEndpoint.register({
    method: "POST",
...
export function getNativeWebSocketMock({
  getSessionUID,
}: {|
    ...
    handler: ({ data }) => {
      ...
      if (messageType === "response" && messageName === "onShippingChange") {
        if (requestUID !== onShippingChangeRequestID) {
          throw new Error(
            `Request uid doest not match for onShippingChange response`,
          );
        }
      }
    },
...
export function mockScript({
  ...
  expect?: boolean,
  block?: boolean,
|}): {| done: () => void, await: () => ZalgoPromise<HTMLElement> |} {
...
document.createElement = function mockCreateElement(name: string): HTMLElement {
  ...
  el.addEventListener = function mockAddEventListener(
    eventName: string,
    handler: () => void,
  ) {
    if (eventName === "load") {
...
export function mockFirebaseScripts(): {|
  done: () => void,
  await: () => ZalgoPromise<$ReadOnlyArray<HTMLElement>>,
|} {
...
export function getNativeFirebaseMock({
  sessionUID,
  extraHandler,
}: {|
  sessionUID: string,
...
|}): NativeMockWebSocket {
    ...
    handler: ({ data }) => {
      for (const id of Object.keys(data)) {
        ...
        if (
          ...
          waitingForResponse.indexOf(requestUID) !== -1
        ) {
        ...
        if (messageType === "request" && messageName === "close") {
          send(
            `users/${sessionUID}/messages/${uniqueID()}`,
            JSON.stringify({
        ...
        if (messageType === "response" && messageName === "onApprove") {
          if (requestUID !== onApproveRequestID) {
            throw new Error(
              `Request uid doest not match for onApprove response`,
            );
          }
        }
        ...
        if (messageType === "response" && messageName === "onCancel") {
          if (requestUID !== onCancelRequestID) {
            throw new Error(
              `Request uid doest not match for onCancel response`,
            );
          }
        }
  ...
  const expect = () => {
    ...
    return {
        ...
        if (waitingForResponse.length) {
          throw new Error(
            `Waiting for responses from firebase: ${waitingForResponse.join(
              ", ",
            )}`,
          );
        }
...
export async function mockSetupButton(
...
export function setCardFieldsValues({
  number,
  expiry,
  cvv,
  name,
...
|}): mixed {
  ...
  const postalCodeInput = postalCode
    ? document.getElementsByName("postal")[0]
    : null;
...
export function getPostRobotMock(): PostRobotMock {
  ...
  const receive = ({ name, win, domain, data }) => {
    ...
    for (const listener of listeners) {
      ...
      if (
        domain &&
        listener.options.domain &&
        listener.options.domain !== domain
      ) {
        continue;
      }

      return ZalgoPromise.delay(5)
        .then(() => listener.handler({ source: win, origin: domain, data }))
        .then(
          (res) => {
            if (listener.promise) {
    ...
    throw new Error(
      `No postRobot handler found for message name: ${name} from ${
        domain || "unset domain"
      }`,
    );
  };
...
export function getMockWindowOpen(
  {
    expectedUrl,
    times = 1,
    appSwitch = false,
    ...
    onOpen = noop,
    expectedQuery = [],
    expectImmediateUrl = true,
  }: MockWindowOptions = getDefaultMockWindowOptions(),
): MockWindow {
  ...
  const done = () => {
    ...
    if (windowOpenedTimes !== times) {
      throw new Error(
        `Expected window.open to be called ${times} times, got ${windowOpenedTimes} calls`,
      );
    }
...
const ensureWindowOpenOnClick = () => {
  ...
  window.HTMLElement.prototype.click =
    function overrideHTMLElementClick(): void {
      doClick();
      return HTMLElementClick.apply(this, arguments);
    };

  const HTMLElementDispatchEvent = window.HTMLElement.prototype.dispatchEvent;
  window.HTMLElement.prototype.dispatchEvent =
    function overrideHTMLElementDispatchEvent(event: Event): void {
      // $FlowFixMe
