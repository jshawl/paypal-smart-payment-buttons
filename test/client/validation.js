...
describe("validation cases", () => {
  it("should render a button, enable the button, click, and call createOrder or onApprove", async () => {
    return await wrapPromise(async ({ expect }) => {
      ...
      window.xprops.onInit = mockAsyncProp(
        expect("onInit", (data, actions) => {
      ...
      window.xprops.createOrder = mockAsyncProp(
        expect("createOrder", () => ZalgoPromise.delay(50).then(() => orderID)),
      );
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", () => ZalgoPromise.resolve()),
      );
  ...
  it("should render a button, disable the button, click, and not call Checkout or createOrder or onApprove", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      window.xprops.createOrder = avoid("createOrder", () =>
        ZalgoPromise.reject(new Error(`Avoid createOrder`)),
      );
      window.xprops.onApprove = avoid("onApprove", () =>
        ZalgoPromise.reject(new Error(`Avoid onApprove`)),
      );
  ...
  it("should render a button, disable the button, click, re-enable the button, click, and call createOrder or onApprove", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      let onClick;
      window.xprops.onClick = ({ fundingSource }, actions) =>
        onClick({ fundingSource }, actions);
      ...
      window.xprops.onInit = mockAsyncProp(
        expect("onInit", (data, actions) => {
          return actions
            .disable()
            .then(async () => {
              onClick = mockAsyncProp(
                expect("onClick", () => ZalgoPromise.resolve()),
              );
              window.xprops.createOrder = avoid("createOrder", () =>
                ZalgoPromise.delay(50).then(() => orderID),
              );
              window.xprops.onApprove = avoid("onApprove", () =>
            ...
            .then(async () => {
              onClick = mockAsyncProp(
                expect("onClick2", () => ZalgoPromise.resolve()),
              );
              window.xprops.createOrder = mockAsyncProp(
                expect("createOrder2", () =>
                  ZalgoPromise.delay(50).then(() => orderID),
              ...
              window.xprops.onApprove = mockAsyncProp(
                expect("onApprove2", () => ZalgoPromise.resolve()),
              );
