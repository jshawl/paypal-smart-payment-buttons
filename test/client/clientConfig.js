      ...
      const gqlMock = getGraphQLApiMock({
        extraHandler: expect("clientConfigCall", ({ data }) => {
          ...
          if (data.variables.integrationArtifact !== "PAYPAL_JS_SDK") {
            throw new Error(
              `Expected integrationArtifact to be PAYPAL_JS_SDK, got ${data.variables.integrationArtifact}`,
            );
          }

          if (data.variables.productFlow !== "SMART_PAYMENT_BUTTONS") {
            throw new Error(
              `Expected productFlow to be SMART_PAYMENT_BUTTONS, got ${data.variables.productFlow}`,
            );
          }

          if (data.variables.userExperienceFlow !== "INCONTEXT") {
            throw new Error(
              `Expected userExperienceFlow to be INCONTEXT, got ${data.variables.userExperienceFlow}`,
            );
          }
      ...
      const gqlMock = getGraphQLApiMock({
        extraHandler: expect("clientConfigCall", ({ data }) => {
          ...
          if (data.variables.userExperienceFlow !== "INLINE") {
            throw new Error(
              `Expected userExperienceFlow to be INLINE, got ${data.variables.userExperienceFlow}`,
            );
          }
      ...
      const gqlMock = getGraphQLApiMock({
        extraHandler: expect("clientConfigCall", ({ data }) => {
          ...
          if (data.variables.fundingSource !== fundingSource) {
            throw new Error(
              `Expected fundingSource to be ${fundingSource}, got ${data.variables.fundingSource}`,
            );
          }
