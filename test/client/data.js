  ...
  it("should render a button, click the button, pass the selected funding to the createOrder data", async () => {
    return await wrapPromise(async ({ expect }) => {
      ...
      window.xprops.createOrder = mockAsyncProp(
        expect("createOrder", async (data) => {
          if (data.paymentSource && data.paymentSource === "venmo") {
            return orderID;
          }

          throw new Error(
            `Expected paymentSource to be available in createOrder data`,
          );
        }),
      );
    ...
    return await wrapPromise(async ({ expect }) => {
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", (data) => {
          ...
          throw new Error(
            `Expected paymentSource to be available in onApprove data`,
          );
        }),
      );
  ...
  it("should render a button, click the button, and pass the selected funding to the createBillingAgreement data", async () => {
    return await wrapPromise(async ({ expect }) => {
      ...
      window.xprops.createBillingAgreement = mockAsyncProp(
        expect("createBillingAgreement", async (data) => {
          if (data.paymentSource && data.paymentSource === "paypal") {
            return billingToken;
          }

          throw new Error(
            `Expected paymentSource to be available in createBillingAgreement data`,
          );
        }),
      );
  ...
  it("should render a button, click the button, and pass the selected funding to the createSubscription data, and render checkout", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      window.xprops.createSubscription = mockAsyncProp(
        expect("createSubscription", async (data) => {
          if (data.paymentSource && data.paymentSource === "paypal") {
            return subscriptionID;
          }

          throw new Error(
            `Expected paymentSource to be available in createSubscription data`,
          );
        }),
      );
