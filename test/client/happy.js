...
describe("happy cases", () => {
      ...
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", async (data) => {
          ...
          if (data.paymentID !== paymentID) {
            throw new Error(
              `Expected paymentID to be ${paymentID}, got ${data.paymentID}`,
            );
          }
        }),
      );
  ...
  it("should render a button with createBillingAgreement, click the button, and render checkout", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      window.xprops.createBillingAgreement = mockAsyncProp(
        expect("createBillingAgreement", async () => {
          return ZalgoPromise.try(() => {
            return billingToken;
          });
        }),
      ...
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", async (data) => {
          if (data.billingToken !== billingToken) {
            throw new Error(
              `Expected billingToken to be ${billingToken}, got ${data.billingToken}`,
            );
          }
  ...
  it("should render a button with createSubscription, click the button, and render checkout", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      window.xprops.vault = true;
      delete window.xprops.createOrder;
      window.xprops.createSubscription = mockAsyncProp(
        expect("createSubscription", async () => {
          return ZalgoPromise.try(() => {
      ...
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", async (data) => {
          if (data.subscriptionID !== subscriptionID) {
            throw new Error(
              `Expected billingToken to be ${subscriptionID}, got ${data.subscriptionID}`,
            );
          }
  ...
  it("should render a button with createSubscription with create call, click the button, and render checkout", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      const subscriptionID = "I-SUBSCRIPTIONID";
      const payerID = "YYYYYYYYYY";
      const createSubscriptionIdApiMock = getCreateSubscriptionIdApiMock(
        {},
        subscriptionID,
      );
      ...
      delete window.xprops.createOrder;
      window.xprops.createSubscription = mockAsyncProp(
        expect("createSubscription", async (data, actions) => {
  ...
  it("should render a button with createSubscription with revise call, click the button, and render checkout", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      const subscriptionID = "I-SUBSCRIPTIONID";
      const payerID = "YYYYYYYYYY";
      const reviseSubscriptionIdApiMock = getReviseSubscriptionIdApiMock(
        {},
        subscriptionID,
      );
    ...
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      triggerKeyPress(
        window.document.querySelector(
          `[data-funding-source=${FUNDING.PAYPAL}]`,
        ),
        13,
      );
    });
  ...
  it("should render a button, click the button, and call onClick", async () => {
    return await wrapPromise(async ({ expect }) => {
      window.xprops.onClick = mockAsyncProp(
        expect("onClick", () => ZalgoPromise.resolve()),
      );
        ...
        expect("Checkout", ({ original: CheckoutOriginal, args: [props] }) => {
          mockFunction(
            ...
            expect(
              ...
              ({ original: onApproveOriginal, args: [data, actions] }) => {
                return onApproveOriginal(
                  { ...data, approveOnClose: true, payerID },
                  actions,
                );
              },
            ),
          ...
          mockFunction(
            ...
            expect("renderTo", async ({ args }) => {
              return props.createOrder().then((id) => {
                ...
                if (approveCalled) {
                  throw new Error(
                    "onApprove is already called but it was only supposed to be called after onClose",
                  );
                }
  ...
  it("should render a button, click the button, and render checkout, then call onApprove prop once", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      mockFunction(
        ...
        expect("Checkout", ({ original: CheckoutOriginal, args: [props] }) => {
          ...
          mockFunction(
            props,
            "onClose",
            expect("onClose", ({ original: onCloseOriginal }) => {
            ...
            expect("renderTo", async ({ args }) => {
              return props.createOrder().then((id) => {
                ...
                if (onApprovePropCalls > 0) {
                  throw new Error(
                    "onApprove prop is already called but it was only supposed to be called after second onApprove callback",
                  );
                }
                approveExpected = true;
      ...
      if (onApprovePropCalls > 1) {
        throw new Error(
          `Expected onApprove to be called once but it was called more than once`,
        );
      }
    });
  });
  ...
  it("should render a button, click the button, and render checkout, onApprove, restart and call onApprove again", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", (data, actions) => onApprove(data, actions)),
      );
  ...
  it("should not error out if no merchant-id is passed in xprops", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      window.xprops.createOrder = mockAsyncProp(
        expect("createOrder", async (data, actions) => {
    ...
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", async (data) => {
          ...
          if (onClickTimes !== 1) {
            throw new Error(
              `Expected onClick to be called 1 time got ${onClickTimes} calls`,
            );
          }
        }),
      );

      window.xprops.onClick = expect(
        "onClick",
        mockAsyncProp(() => {
          onClickTimes += 1;
        }),
