...
describe("popup bridge cases", () => {
  it("should render a button with createOrder, click the button, and render checkout using popup bridge", async () => {
    return await wrapPromise(async ({ expect }) => {
      ...
      window.xprops.getPopupBridge = mockAsyncProp(
        expect("getPopupBridge", async () => {
          return {
            nativeUrl,
            start: expect("start", async (url) => {
              ...
              if (query.token !== orderID) {
                throw new Error(
                  `Expected token to be ${orderID}, got ${query.token}`,
                );
              }

              if (query.useraction !== "commit") {
                throw new Error(
                  `Expected useraction to be commit, got ${query.useraction}`,
                );
              }

              if (query.fundingSource !== FUNDING.PAYPAL) {
                throw new Error(
                  `Expected fundingSource to be ${FUNDING.PAYPAL}, got ${query.fundingSource}`,
                );
              }
              ...
              if (query.native_xo !== "1") {
                throw new Error(
                  `Expected native_xo to be 1, got ${query.native_xo}`,
                );
              }
      ...
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", async (data) => {
          ...
          if (data.billingToken !== billingToken) {
            throw new Error(
              `Expected paymentID to be ${billingToken}, got ${data.billingToken}`,
            );
          }
        }),
      );
