...
const EXPECTED_POPUP_URL =
  "https://history.paypal.com/smart/checkout/native/popup";
const EXPECTED_VENMO_POPUP_URL =
  "https://history.paypal.com/smart/checkout/venmo/popup";
const EXPECTED_MOBILE_POPUP_URL =
  "https://mobile.paypal.com/smart/checkout/native/popup";
...
const EXPECTED_NATIVE_FALLBACK_URL =
  "https://www.paypal.com/smart/checkout/fallback";
...
const mockWebCheckout = ({
  ...
  const checkoutMock = mockFunction(
    window.paypal,
    "Checkout",
    expect(
      "Checkout",
      ({ original: CheckoutOriginal, args: [checkoutProps] }) => {
        if (mockWindow) {
          ...
          if (actualMockWindow && actualPropWindow !== actualMockWindow) {
            throw new Error(
              `Expected win passed to checkout to match win sent in onLoad`,
            );
          }
          ...
          if (checkoutProps.window) {
            throw new Error(
              `Expected no window to be passed in checkout props`,
            );
          }
        }
...
describe("native ios/safari cases", () => {
  ...
  it("should render a button with createOrder, click the button, and render checkout via popup to web path in iOS with multiple clicks", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", (innerData) => {
          if (innerData.orderID !== orderID) {
            throw new Error(
              `Expected orderID to be ${orderID}, got ${innerData.orderID}`,
            );
          }
        }),
      );
  ...
  it("should render a button with createOrder and onClick resolving, click the button, and render checkout via popup to native path in iOS", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      window.xprops.onClick = mockAsyncProp(
        expect("onClick", async (data, actions) => {
  ...
  it("should render a button with createOrder and onClick rejecting, click the button, and render checkout via popup to native path in iOS", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      window.xprops.onCancel = mockAsyncProp(
        avoid(POST_MESSAGE.ON_CANCEL, promiseNoop),
      );
      window.xprops.onApprove = mockAsyncProp(
        avoid(POST_MESSAGE.ON_APPROVE, promiseNoop),
      );
      window.xprops.onError = avoid(POST_MESSAGE.ON_ERROR);
  ...
  it("should render a button with eligibility rejecting, click the button, and render checkout via popup to web path in iOS", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      const mockWindow = getMockWindowOpen({
        ...
        onOpen: () => {
          sendRedirectMessage({
            mockWindow,
            expect,
            url: EXPECTED_NATIVE_FALLBACK_URL,
      ...
      const gqlMock = getEligibilityGraphQLMock({
        expect,
        eligibility: {
          paypal: {
            eligibility: false,
  ...
  it("should render a button with createOrder, click the button, and render checkout via popup to web path in iOS, then retry on native", async () => {
    return await wrapPromise(async ({ expect, avoid }) => {
      ...
      const mockWindow = getMockWindowOpen({
        ...
        onOpen: () => {
          sendRedirectMessage({ mockWindow, expect }).then(
            ({ url: redirectUrl, query: redirectQuery }) => {
              ZalgoPromise.try(() => {
                if (!appInstalled) {
        ...
        onOpen: () => {
          sendRedirectMessage({ mockWindow, expect }).then(
            ({ query: redirectQuery }) => {
              ...
              const { expect: expectSocket, onFallback } =
                getNativeFirebaseMock({
                  sessionUID: redirectQuery.sessionUID,
                });
        ...
        extraHandler: expect("firebaseGQLCall", ({ data }) => {
          ...
          if (data.query.includes("mutation UpdateClientConfig")) {
            if (data.variables.buttonSessionID) {
              ...
              throw new Error(
                `Expected button session id to be present in UpdateClientConfig call`,
              );
            }
          }
        }),
      ...
      window.xprops.onApprove = mockAsyncProp(
        expect("onApprove", (data) => {
          ...
          if (!clientConfigCorrectlyCalled) {
            throw new Error(
              `Expected UpdateClientConfig call to go through correctly`,
            );
          }
        }),
      );
