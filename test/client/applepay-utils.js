...
} from "../../src/payment-flows/applepay/utils";
...
describe("Apple Pay Flow Utils", () => {
  describe("isJSON", () => {
    it("should be able to check for valid JSON data structure", () => {
      ...
      if (!isValidJSON) {
        throw new Error(
          `Expected ${JSON.stringify(json)} to be valid JSON format.`,
        );
      }
    });
    ...
    it("should be able to check for invalid JSON data structure", () => {
      ...
      if (isValidJSON) {
        throw new Error(
          `Expected ${JSON.stringify(json)} to not be valid JSON format.`,
        );
      }
    });
  });
  ...
  describe("validateShippingContact", () => {
    it("should validate happy shipping contact", () => {
      ...
      if (errors && errors.length) {
        throw new Error(
          `Shipping contact, ${JSON.stringify(
            shippingContact,
      ...
      if (
        ...
        throw new Error(
          `Invalid returned shipping_address. ${JSON.stringify(
            shipping_address,
          )}`,
        );
      }
