    ...
    it("should render a single card fields with createOrder", async () => {
      return await wrapPromise(async ({ expect }) => {
        ...
        mockFunction(
          ...
          expect(
            "CardFields",
            ({ original: CardFieldOriginal, args: [props] }) => {
              ...
              mockFunction(
                ...
                expect(
                  "render",
                  async ({ original: renderToOriginal, args }) => {
          ...
          extraHandler: ({ data }) => {
            ...
            if (data.query.includes("mutation ProcessPayment")) {
              ...
              if (data.variables.card.cardNumber !== "5555555555554444") {
                throw new Error(
                  `Expected card number to be 5555555555554444 but got ${data.variables.card.cardNumber}`,
                );
              }

              if (data.variables.card.expirationDate !== "01/2022") {
                throw new Error(
                  `Expected expiry to be 01/2022 but got ${data.variables.card.expirationDate}`,
                );
              }

              if (data.variables.card.securityCode !== "123") {
                throw new Error(
                  `Expected CVV to be 123 but got ${data.variables.card.securityCode}`,
                );
              }
      ...
      return await wrapPromise(async ({ expect, wait }) => {
        ...
        const gqlMock = getGraphQLApiMock({
          extraHandler: ({ data }) => {
            ...
            if (data.query.includes("mutation ProcessPayment")) {
              throw new Error(
                "Expected to not call ProcessPayment if the card data is invalid",
              );
            }
          },
        }).listen();
            ...
            ({ original: CardFieldOriginal, args: [props] }) => {
              ...
              setTimeout(() => {
                cardFieldInstance.submit().catch((error) => {
                  if (!error) {
                    throw new Error(
                      "Expected on submit error if the card data is invalid",
                    );
                  }
                });
              }, 10);
          ...
          expect(
            ...
            ({ original: CardFieldOriginal, args: [props] }) => {
              ...
              mockFunction(
                ...
                expect(
                  "NumberField",
                  ({ original: NumberFieldToOriginal, args: numberArgs }) => {
                    const numberFieldToOriginal = NumberFieldToOriginal(
                      ...numberArgs,
                    );

                    mockFunction(
                      numberFieldToOriginal,
                      "render",
                      expect(
                        "render",
                        async ({ original: renderToOriginal, args }) => {
              ...
              mockFunction(
                ...
                expect(
                  "ExpiryField",
                  ({ original: ExpiryFieldToOriginal, args: expiryArgs }) => {
                    const expiryFieldToOriginal = ExpiryFieldToOriginal(
                      ...expiryArgs,
                    );

                    mockFunction(
                      expiryFieldToOriginal,
                      "render",
                      expect(
                        "render",
                        async ({ original: renderToOriginal, args }) => {
              ...
              mockFunction(
                ...
                expect(
                  "CVVField",
                  ({ original: CVVFieldToOriginal, args: cvvArgs }) => {
          ...
          extraHandler: ({ data }) => {
            ...
            if (data.query.includes("mutation ProcessPayment")) {
              ...
              if (data.variables.card.cardNumber !== cardNumber) {
                throw new Error(
                  `Expected card number to be ${cardNumber} but got ${data.variables.card.cardNumber}`,
                );
              }

              if (data.variables.card.expirationDate !== cardExpiry) {
                throw new Error(
                  `Expected expiry to be ${cardExpiry} but got ${data.variables.card.expirationDate}`,
                );
              }

              if (data.variables.card.securityCode !== cardCvv) {
                throw new Error(
                  `Expected CVV to be ${cardCvv} but got ${data.variables.card.securityCode}`,
                );
              }
              ...
              mockFunction(
                ...
                expect(
                  ...
                  ({ original: ExpiryFieldToOriginal, args: expiryArgs }) => {
                    ...
                    renderCardFieldMock({
                      name: "card-expiry-field",
                      isFieldValid: () => true,
                      getFieldValue: () => cardExpiry,
                    });
          ...
          expect(
            ...
            ({ original: CardFieldOriginal, args: [props] }) => {
              ...
              mockFunction(
                ...
                expect(
                  "NameField",
                  ({ original: NameFieldToOriginal, args: nameArgs }) => {
                    const nameFieldToOriginal = NameFieldToOriginal(
                      ...nameArgs,
                    );
              ...
              mockFunction(
                ...
                expect(
                  "PostalCodeField",
                  ({
                    original: PostalCodeFieldToOriginal,
                    args: postalCodeArgs,
                  }) => {
                    const postalCodeFieldToOriginal = PostalCodeFieldToOriginal(
                      ...postalCodeArgs,
                    );

                    mockFunction(
                      postalCodeFieldToOriginal,
                      "render",
                      expect(
                        "render",
                        async ({ original: renderToOriginal, args }) => {
                    ...
                    renderCardFieldMock({
                      name: "card-postal-field",
                      isFieldValid: () => true,
                      getFieldValue: () => cardPostalCode,
                    });
        ...
        expect(
          ...
          ({ original: CardFieldOriginal, args: [props] }) => {
            ...
            if (numberInput.getAttribute("autocomplete") !== "cc-number") {
              throw new Error(
                'Card Number autocomplete is not set to "cc-number"',
              );
            }

            if (expiryInput.getAttribute("autocomplete") !== "cc-exp") {
              throw new Error(
                'Card Expiry autocomplete is not set to "cc-exp"',
              );
            }
