...
import { mockSetupButton, mockAsyncProp, createButtonHTML, clickButton, getMockWindowOpen,
    mockFunction, getGraphQLApiMock, type MockWindow } from './mocks';
...
const EXPECTED_VENMO_POPUP_QUERY_PARAMS = [ 'sdkMeta', 'buttonSessionID', 'parentDomain', 'venmoWebEnabled', 'venmoWebUrl' ];
...
const mockOnError = ({ expect }) => {
    window.xprops.onError = mockAsyncProp(expect('onError', async () => {
        return ZalgoPromise.resolve(true);
    }), 50);
};

const mockOnCancel = ({ expect }) => {
    window.xprops.onCancel = mockAsyncProp(expect('onCancel', async () => {
        return ZalgoPromise.resolve(true);
    }), 50);
};

const mockOnComplete = ({ expect }) => {
    window.xprops.onComplete = mockAsyncProp(expect('onComplete', async () => {
        return ZalgoPromise.resolve(true);
    }), 50);
};
...
const mockVenmoWebCheckout = ({ expect, error = false, cancel = false, complete = false, orderID, payerID, mockWindow, approve = true, onComplete = noop } : MockWebCheckoutOptions) => {
    const venmoMock = mockFunction(window.paypal, 'Venmo', expect('Venmo', ({ original: CheckoutOriginal, args: [ props ] }) => {
        const checkoutInstance = CheckoutOriginal(props);

        const renderToMock = mockFunction(checkoutInstance, 'renderTo', expect('renderTo', async ({ args }) => {
        ...
        it('should render a button with createOrder, click the button, and detect web switch in iOS and restart with Venmo web', async () => {
            return await wrapPromise(async ({ expect, avoid }) => {
                ...
                const mockWindow = getMockWindowOpen({
                    ...
                    expectClose:   false,
                    onOpen:        () => {
                        sendRedirectMessage({ mockWindow, expect, url: EXPECTED_VENMO_URL }).then(({ url: redirectUrl }) => {
